...
from HydraServer.db.model import Template, TemplateType, TypeAttr, Attr, Network, Node, Link, ResourceGroup, \
    ResourceType, ResourceAttr, ResourceScenario, Scenario, TemplateOwner
...
def get_types_by_attr(resource, template_id=None):
    ...
    all_types = DBSession.query(TemplateType).options(joinedload_all('typeattrs')).filter(
        TemplateType.resource_type == resource.ref_key)
...
def _get_attr_by_name_and_dimension(name, dimension):
    ...
    attr = DBSession.query(Attr).filter(Attr.attr_name.ilike(name), Attr.attr_dimen.ilike(dimension)).first()
...
def upload_template_xml(template_xml, **kwargs):
    ...
    try:
        tmpl_i = DBSession.query(Template).filter(Template.template_name == template_name).options(
            joinedload_all('templatetypes.typeattrs.attr')).one()
    ...
    for resource in types.findall('resource'):
        ...
        if type_name in existing_types:
            ...
            type_i = DBSession.query(TemplateType).filter(TemplateType.type_id == type_id).options(
                joinedload_all('typeattrs.attr')).one()
        ...
        for attr_to_delete in attrs_to_delete:
            ...
            try:
                attr_i = DBSession.query(TypeAttr).filter(TypeAttr.attr_id == attr_id,
                                                          TypeAttr.type_id == type_id).options(
                    joinedload_all('attr')).one()
...
def assign_types_to_resources(resource_types, **kwargs):
    ...
    db_types = DBSession.query(TemplateType).filter(TemplateType.type_id.in_(type_ids)).options(
        joinedload_all('typeattrs')).all()
...
def check_type_compatibility(type_1_id, type_2_id):
    ...
    type_1 = DBSession.query(TemplateType).filter(TemplateType.type_id == type_1_id).options(
        joinedload_all('typeattrs')).one()
    type_2 = DBSession.query(TemplateType).filter(TemplateType.type_id == type_2_id).options(
        joinedload_all('typeattrs')).one()
...
def _get_links(link_ids):
    ...
    if len(link_ids) > 500:
        ...
        while idx < len(link_ids):
            ...
            rs = DBSession.query(Link).options(joinedload_all('attributes')).options(joinedload_all('types')).filter(
                Link.link_id.in_(link_ids[idx:extent])).all()
        ...
        links = DBSession.query(Link).options(joinedload_all('attributes')).options(joinedload_all('types')).filter(
            Link.link_id.in_(link_ids)).all()
...
def _get_nodes(node_ids):
    ...
    if len(node_ids) > 500:
        ...
        while idx < len(node_ids):
            ...
            rs = DBSession.query(Node).options(joinedload_all('attributes')).options(joinedload_all('types')).filter(
                Node.node_id.in_(node_ids[idx:extent])).all()
        ...
        nodes = DBSession.query(Node).options(joinedload_all('attributes')).options(joinedload_all('types')).filter(
            Node.node_id.in_(node_ids)).all()
...
def _get_groups(group_ids):
    ...
    if len(group_ids) > 500:
        ...
        while idx < len(group_ids):
            ...
            rs = DBSession.query(ResourceGroup).options(joinedload_all('attributes')).filter(
                ResourceGroup.group_id.in_(group_ids[idx:extent])).all()
        ...
        groups = DBSession.query(ResourceGroup).options(joinedload_all('types')).options(
            joinedload_all('attributes')).filter(ResourceGroup.group_id.in_(group_ids))
    group_dict = {}
...
def get_template(template_id, **kwargs):
    ...
    tmpl = DBSession.query(Template).filter(Template.template_id == template_id).options(
        joinedload_all('templatetypes.typeattrs.default_dataset.metadata')).one()
...
def get_templatetype_by_name(template_id, type_name, **kwargs):
    ...
    try:
        templatetype = DBSession.query(TemplateType).filter(TemplateType.template_id == template_id,
                                                            TemplateType.type_name == type_name).one()
    except NoResultFound:
...
def validate_network(network_id, template_id, scenario_id=None):
    ...
    template = DBSession.query(Template).filter(Template.template_id == template_id).options(
        joinedload_all('templatetypes')).first()
